...
void RespondToCommand(tflite::ErrorReporter* error_reporter,
    ...
    if (ready_ == 0){
      ...
      if (ready_ == 1){
         ...
         if (found_command[0] == 'l') {
          last_command_time = current_time;
          digitalWrite(4, HIGH);  // Turn on  light
          digitalWrite(LEDG, HIGH);
          ready_ = 0;
          ...
          last_command_time = current_time;
          digitalWrite(6, HIGH);  // Turn on  heater
          digitalWrite(LEDG, HIGH);
          ready_ = 0;
        }
      }
  ...
  // If last_command_time is non-zero but no command to turn on or in0 >5 seconds ago, zero it
